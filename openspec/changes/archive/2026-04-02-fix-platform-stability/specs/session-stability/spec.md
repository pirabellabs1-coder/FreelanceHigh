...
The middleware SHALL treat all routes under `/formations/` as a separate routing domain. A user with main app role `client` navigating formations apprenant pages SHALL NOT be redirected to `/client`. The formations space SHALL only enforce `formationsRole` checks, never `role` checks (except for `/formations/admin` which requires `role=admin`).

#### Scenario: Client-role user browsing formations as apprenant
- **WHEN** a user with `role=client` and valid session navigates to `/formations/mes-formations`
...
- **WHEN** a user with valid session navigates to any route starting with `/formations/` that is not public, auth, or instructeur-scoped
...
- **WHEN** a developer adds a new page `/formations/remboursement` or `/formations/historique`
- **THEN** the middleware SHALL still protect it (require auth) because the catch-all guard covers all `/formations/` non-public routes
...
- **WHEN** an unauthenticated user navigates to `/formations` or `/formations/[slug]`
- **THEN** the middleware SHALL allow access without redirect

#### Scenario: Unauthenticated user accessing protected formation route
- **WHEN** an unauthenticated user navigates to `/formations/mes-formations`
- **THEN** the middleware SHALL redirect to `/formations/connexion`
