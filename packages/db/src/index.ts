// Lazy Prisma client — evite "@prisma/client did not initialize" pendant le build Next.js
...
function getPrisma(): PrismaClientType {
  if (!_prisma) {
    ...
    const { PrismaClient } = require("@prisma/client") as { PrismaClient: new (opts?: Record<string, unknown>) => PrismaClientType };
...
// Re-export des types Prisma (types purs, pas d'initialisation)
