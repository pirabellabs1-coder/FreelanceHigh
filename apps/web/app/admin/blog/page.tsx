...
export default function AdminBlog() {
  ...
  const safeArticles = blogArticles || [];

  const filtered = useMemo(() => {
    if (tab === "tous") return safeArticles;
    return safeArticles.filter(a => a.status === tab);
  }, [tab, safeArticles]);

  const stats = useMemo(() => ({
    publie: safeArticles.filter(a => a.status === "publie").length,
    brouillon: safeArticles.filter(a => a.status === "brouillon").length,
    programme: safeArticles.filter(a => a.status === "programme").length,
    totalViews: safeArticles.reduce((s, a) => s + (a.views ?? 0), 0),
  }), [safeArticles]);
  ...
  function openEdit(article: AdminBlogArticle) {
    setForm({
      ...
      category: article.category || "Conseils", tags: [...(article.tags || [])], author: article.author || "",
      status: article.status || "brouillon", scheduledAt: article.scheduledAt ?? null,
  ...
  async function handleDelete() {
    if (!deleteId) return;
    const article = safeArticles.find(a => a.id === deleteId);
    setSaving(true);
    try {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{safeArticles.length} articles — {stats.totalViews.toLocaleString()} vues totales</p>
        </div>
      ...
      <div className="flex gap-2 border-b border-border-dark overflow-x-auto">
        {[
          { key: "tous", label: "Tous", count: safeArticles.length },
      ...
      {/* Articles — mobile cards + desktop table */}
      {/* Mobile card layout */}
      <div className="space-y-3 md:hidden">
        {filtered.map(a => (
          <div key={a.id} className="bg-neutral-dark rounded-xl border border-border-dark p-4">
            <div className="flex items-start justify-between gap-2 mb-2">
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-white truncate">{a.title}</p>
                <p className="text-xs text-slate-500 truncate">par {a.author} — /{a.slug}</p>
              </div>
              <span className={cn("text-xs font-semibold px-2 py-0.5 rounded-full shrink-0", STATUS_MAP[a.status]?.cls)}>{STATUS_MAP[a.status]?.label}</span>
            </div>
            <div className="flex items-center gap-3 text-xs text-slate-400 mb-3">
              <span className="bg-border-dark text-slate-300 px-2 py-0.5 rounded-full">{a.category}</span>
              <span>{(a.views ?? 0) > 0 ? `${(a.views ?? 0).toLocaleString()} vues` : "0 vues"}</span>
      ...
      <div className="bg-neutral-dark rounded-xl border border-border-dark overflow-hidden hidden md:block">
            ...
            <tbody>
              {filtered.map(a => (
                <tr key={a.id} className="border-b border-border-dark/50 hover:bg-background-dark/30 transition-colors">
                  ...
                  <td className="px-5 py-3 text-sm text-center font-bold text-white">{(a.views ?? 0) > 0 ? (a.views ?? 0).toLocaleString() : "—"}</td>
                  <td className="px-5 py-3 text-sm text-slate-400">{a.createdAt ? new Date(a.createdAt).toLocaleDateString("fr-FR") : "—"}</td>
                  <td className="px-5 py-3">
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setModal(null)}>
          <div onClick={e => e.stopPropagation()} className={cn("bg-neutral-dark rounded-2xl p-4 sm:p-6 w-full border border-border-dark shadow-2xl overflow-y-auto transition-all mx-2 sm:mx-0", fullscreen ? "max-w-none sm:m-4 max-h-[calc(100vh-1rem)] sm:max-h-[calc(100vh-2rem)]" : "max-w-3xl max-h-[90vh]")}>
