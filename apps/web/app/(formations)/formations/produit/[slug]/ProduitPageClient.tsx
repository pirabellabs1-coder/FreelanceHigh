    ...
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center px-6">
        <div className="bg-white rounded-lg border border-gray-100 shadow-sm p-12 text-center max-w-md">
          ...
          <h2 className="text-base md:text-lg font-bold text-gray-900 mt-3">Produit introuvable</h2>
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 md:px-8 py-3">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-xs text-gray-600">
          <Link href="/formations" className="hover:text-fh-600 transition-colors">Accueil</Link>
          <span className="material-symbols-outlined text-[12px]">chevron_right</span>
          <Link href="/formations/explorer" className="hover:text-fh-600 transition-colors">Explorer</Link>
          <span className="material-symbols-outlined text-[12px]">chevron_right</span>
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-5">
            {/* Banner — sobre, solid bg-fh-600 (sans gradient) */}
            <div className="relative aspect-video rounded-lg overflow-hidden bg-fh-600">
              {product.banner ? (
              ...
              <div className="absolute top-4 left-4 flex items-center gap-2">
                <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full bg-white/95 text-gray-900 shadow-sm backdrop-blur-sm">
                  <span className="material-symbols-outlined text-[12px] text-fh-600">{typeInfo.icon}</span>
                  {typeInfo.label}
                </span>
                {product.category && (
                  <span className="text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full bg-fh-600 text-white">
            ...
            <div className="bg-white rounded-lg border border-gray-100 shadow-sm p-5 md:p-6">
              <div className="flex flex-wrap items-center gap-2 mb-3">
                {product.tags && product.tags.slice(0, 4).map((t) => (
                  <span key={t} className="inline-block text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-gray-100 text-gray-600">
                    #{t}
                  </span>
                ))}
              ...
              <h1 className="text-xl md:text-2xl font-extrabold text-gray-900 leading-tight">{product.title}</h1>
              ...
              <div className="flex items-center gap-4 mt-4 flex-wrap">
                <div className="flex items-center gap-1.5">
                  ...
                  {product.reviewsCount > 0 && (
                    <span className="text-xs text-gray-500">({product.reviewsCount} avis)</span>
                  )}
                </div>
                <span className="text-xs text-gray-500 flex items-center gap-1">
                  <span className="material-symbols-outlined text-[14px]">shopping_bag</span>
                  <span className="font-semibold text-gray-900">{fmt(product.salesCount)}</span>
              ...
              {(["description", "avis"] as const).map((tab) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-md text-sm font-semibold transition-colors ${
                    activeTab === tab ? "bg-fh-600 text-white shadow-sm" : "text-gray-600 hover:text-gray-900"
                  }`}
                >
            ...
            {activeTab === "description" && (
              <div className="space-y-5">
                {product.description ? (
                  <div className="bg-white rounded-lg border border-gray-100 shadow-sm p-5 md:p-6">
                    <h2 className="text-base md:text-lg font-extrabold text-gray-900 mb-3">À propos de ce produit</h2>
                    {/* Render HTML if description contains tags, otherwise plain text */}
                    {product.description.includes("<") && product.description.includes(">") ? (
                      <div
                        className="text-sm text-gray-600 leading-relaxed prose prose-sm max-w-none prose-headings:text-gray-900 prose-strong:text-gray-900 prose-ul:my-2 prose-li:my-0.5"
                  ...
                  <div className="bg-white rounded-lg border border-gray-100 shadow-sm p-8 text-center">
                    ...
                    <p className="text-sm text-gray-600 mt-2">Aucune description fournie pour ce produit.</p>
                  </div>
                )}

                {/* Seller card */}
                <div className="bg-white rounded-lg border border-gray-100 shadow-sm p-5 md:p-6">
                  <h2 className="text-base md:text-lg font-extrabold text-gray-900 mb-4">À propos du créateur</h2>
                  <Link href={`/formations/instructeurs/${product.instructeur.userId}`} className="flex items-start gap-4 group">
                    <div className="w-14 h-14 rounded-full overflow-hidden bg-fh-600 flex items-center justify-center text-white font-bold text-base md:text-lg flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-gray-900 group-hover:text-fh-600 transition-colors">
                      ...
                      {product.instructeur.bio && (
                        <p className="text-xs text-gray-600 mt-2 leading-relaxed line-clamp-3">{product.instructeur.bio}</p>
                      )}
                      {product.instructeur.expertise && product.instructeur.expertise.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-3">
                          {product.instructeur.expertise.slice(0, 4).map((e) => (
                            <span key={e} className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-fh-600/10 text-fh-600">
                              {e}
                            </span>
                          ))}
                        </div>
                    ...
                    <span className="material-symbols-outlined text-gray-500 text-[18px] group-hover:text-fh-600 transition-colors">chevron_right</span>
                  </Link>
                </div>
              </div>
            )}
              ...
              <div className="bg-white rounded-lg border border-gray-100 shadow-sm p-5 md:p-6">
                {product.reviews.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-gray-600 mt-3">Aucun avis pour ce produit pour l&apos;instant.</p>
                    <p className="text-xs text-gray-500 mt-1">Soyez le premier à laisser votre retour après l&apos;achat.</p>
                  </div>
                ) : (
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <p className="text-sm font-bold text-gray-900">{r.user.name ?? "Acheteur"}</p>
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-lg border border-gray-100 shadow-sm overflow-hidden sticky top-4">
              <div className="p-5">
                <div className="mb-4">
                  ...
                  <div className="flex items-baseline gap-2">
                    {isFree ? (
                      ...
                      <>
                        <p className="text-xl md:text-2xl font-extrabold text-fh-600">{fmt(product.price)}</p>
                  ...
                  {!isFree && (
                    <p className="text-xs text-gray-500 mt-1">≈ {Math.round(product.price / 655.957)} EUR</p>
                  )}
                </div>
                ...
                <div className="mt-5 pt-5 border-t border-gray-100 space-y-2.5">
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-600">
                    <span className="material-symbols-outlined text-fh-600 text-[16px]">{typeInfo.icon}</span>
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-600">
                    <span className="material-symbols-outlined text-fh-600 text-[16px]">verified_user</span>
            ...
            >
              <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-2">Créé par</p>
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold text-gray-900 truncate">{product.instructeur.name ?? "Créateur"}</p>
                  {product.rating > 0 && (
