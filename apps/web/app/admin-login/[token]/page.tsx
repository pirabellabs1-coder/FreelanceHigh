...
export default function SecureAdminLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Admin verified — don't reset loading, keep spinner until redirect
      router.push("/admin");
      router.refresh();
    } catch {
  ...
  // Token invalid — will redirect to 404
      ...
      <div className="w-full max-w-sm">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl">
          ...
          <div className="text-center mb-8">
            <div className="w-14 h-14 rounded-xl bg-red-600/10 flex items-center justify-center mx-auto mb-4">
              <span className="material-symbols-outlined text-2xl text-red-500">shield</span>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 active:scale-[0.98] transition-all disabled:opacity-50 disabled:hover:bg-red-600 flex items-center justify-center gap-2 mt-2"
            >
              {loading ? (
                <>
          ...
          {/* Quick access to both admin spaces after login */}
          <div className="mt-6 pt-4 border-t border-slate-800">
            <p className="text-slate-600 text-[10px] text-center uppercase tracking-widest">Espaces administres</p>
            <div className="flex gap-2 mt-2">
              <div className="flex-1 text-center py-2 px-2 bg-slate-800/50 rounded-lg">
                <span className="text-slate-500 text-xs">Marketplace</span>
              </div>
              <div className="flex-1 text-center py-2 px-2 bg-slate-800/50 rounded-lg">
                <span className="text-slate-500 text-xs">Formations</span>
              </div>
            </div>
          </div>
        </div>

        <p className="text-slate-700 text-[10px] text-center mt-4">
