...
export default function FunnelsListPage() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const json = await res.json();
      router.push(`/formations/vendeur/marketing/funnels/${json.data.id}`);
    } catch (e) {
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-sm text-[#5c647a] mb-2">
            <Link href="/formations/vendeur/marketing" className="hover:text-[#006e2f] transition-colors">
              Marketing
            </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {funnels.map((f) => {
            ...
            return (
              <Link
                key={f.id}
                href={`/formations/vendeur/marketing/funnels/${f.id}`}
