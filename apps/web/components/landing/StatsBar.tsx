...
export function StatsBar() {
  ...
  // Show actual numbers — even small counts are honest for an early-stage platform
  const freelanceDisplay = stats ? (formatStatNumber(freelanceCount) ?? String(freelanceCount)) : null;
  const ordersDisplay = stats ? (formatStatNumber(completedOrders) ?? String(completedOrders)) : null;
  ...
  return (
    <section className="px-4 sm:px-6 lg:px-8 -mt-10 sm:-mt-20 relative z-20">
      <div className="max-w-[1440px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
        {/* Freelances actifs */}
